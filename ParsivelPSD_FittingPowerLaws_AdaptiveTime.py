...
def calc_dsd(date):
    ...
    parsivel_data = xr.open_mfdataset('/data/accp/a/mp46/Parsivel_data/DSD/'+date+'_DSD_BUF.nc')

    gauge_and_wx_station_data = xr.open_mfdataset('/data/accp/a/mp46/gauge_wx_sensor_filtered_data_BUF.nc')
    
    time_ds1 = parsivel_data.time.values
    ...
    # Find the indices of common timestamps in the gauge data
    common_indices_ds2 = np.where(np.isin(time_ds2, common_timestamps))[0]
    
    # If the length of the parsivel data file doesn't match the length of the gauge data (missing gauge obs)
    # then find the timestamps common to both datasets and only use those parsivel data points going forward
    if not len(parsivel_data.time) == len(gauge_and_wx_station_data.UTC[common_indices_ds2[0]:(common_indices_ds2[-1]+1)]):
        gauge_data = gauge_and_wx_station_data.UTC[common_indices_ds2[0]:(common_indices_ds2[-1]+1)]
        # Find the common times between ds1 and ds2
        common_times = parsivel_data['time'].isin(gauge_data['UTC'])

        # Select data in ds1 that is also found in ds2
        ds = parsivel_data.sel(time=common_times)
    else:
        ds = parsivel_data
