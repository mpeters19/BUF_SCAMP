...
def calc_dsd(date):
    ...
    '''
    ds_10s = xr.open_mfdataset('/data/accp/a/snesbitt/scamp/parsivel/nc_daily/'+date+'_*_SCAMP_Parsivel.nc') #opens the data files
...
class ParsivelPSDBUF(object):
    ...
    def __init__(self,ds):
        ...
        self.mass_spectrum_warm = (np.zeros(self.timedim),np.zeros((self.timedim,32))) #mass spectrum using Heymsfield warm coefficients 
        self.mass_spectrum_cold = (np.zeros(self.timedim),np.zeros((self.timedim,32)))  #mass spectrum using Heymsfield cold coefficients 
        self.mass_spectrum_conv = (np.zeros(self.timedim),np.zeros((self.timedim,32))) #mass spectrum using Heymsfield convective coefficients 
